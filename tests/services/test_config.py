...
from services.config.settings import ProviderConfig, Settings
from services.core.exceptions import ServiceError
...
class TestProviderConfig(unittest.TestCase):
    ...
    def test_provider_config_creation(self):
        ...
        config = ProviderConfig(provider_type="test", enabled=True, config={"key": "value"})
...
class TestServiceFactory(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.settings.storage_configs["local"].config.update({"base_path": "/tmp/test_storage"})
        self.settings.transcription_configs["openai"].config.update(
            {"api_key": "test_key", "model": "whisper-1"}
        )
        self.settings.job_runner_configs["local"].config.update({"work_dir": "/tmp/test_jobs"})
