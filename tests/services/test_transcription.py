...
from services.core.exceptions import AuthenticationError, TranscriptionError
...
class TestTranscriptionService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.service = TranscriptionService(
            transcription_provider=self.mock_provider, storage_provider=self.mock_storage
        )
    ...
    def test_transcribe_local_file_success(self):
        ...
        try:
            ...
            self.mock_provider.get_supported_formats = AsyncMock(return_value=[".mp3", ".wav"])

            mock_result = TranscriptionResult(
                text="This is the transcribed text",
                segments=[
                    TranscriptionSegment(
                        id=0, start=0.0, end=5.0, text="This is the transcribed text"
                    )
                ],
                language="en",
                duration=5.0,
    ...
    def test_transcribe_from_storage(self):
        ...
        self.mock_storage.download_file = AsyncMock(
            return_value=Mock(success=True, local_path="/tmp/downloaded.mp3")
        )

        # Mock provider responses
        self.mock_provider.get_max_file_size = AsyncMock(return_value=25 * 1024 * 1024)
        self.mock_provider.get_supported_formats = AsyncMock(return_value=[".mp3"])
        ...
        try:
            ...
            self.mock_storage.download_file = AsyncMock(
                return_value=Mock(success=True, local_path=tmp_path)
            )

            async def run_test():
                result = await self.service.transcribe_file("/storage/test.mp3", from_storage=True)
    ...
    def test_process_and_store(self):
        ...
        file_info = FileInfo(
            path="/storage/test.mp3", name="test.mp3", size=1024, modified_at=datetime.now()
        )
        ...
        with patch.object(
            self.service, "transcribe_file", new_callable=AsyncMock
        ) as mock_transcribe:
            mock_transcribe.return_value = mock_result

            # Mock storage uploads
            self.mock_storage.upload_file = AsyncMock(return_value=Mock(success=True))
    ...
    def test_transcribe_success(self, mock_executor):
        ...
        try:
            ...
            mock_executor.submit = Mock(
                side_effect=lambda fn, *args: Mock(result=lambda: fn(*args))
            )
            ...
            async def run_test():
                ...
                with patch.object(asyncio.get_event_loop(), "run_in_executor") as mock_run:
                    ...
                    expected_result = TranscriptionResult(
                        ...
                        segments=[
                            TranscriptionSegment(id=0, start=0.0, end=5.0, text="Transcribed text")
                        ],
                        language="en",
                        duration=5.0,
                        processed_at=datetime.now(),
                    ...
                    with patch.object(
                        self.provider, "transcribe", new_callable=AsyncMock
                    ) as mock_transcribe:
...
class TestTranscriptionProviderContract(unittest.TestCase):
    ...
    def test_openai_provider_implements_interface(self):
        ...
        required_methods = ["transcribe", "get_supported_formats", "get_max_file_size"]
