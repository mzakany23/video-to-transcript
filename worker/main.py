...
class TranscriptionJobProcessor:
    ...
    def __init__(self):
        ...
        print("🔧 Initialized transcription processor with Dropbox")
    ...
    def process_single_file(self, file_path: str, file_name: str):
        ...
        try:
            ...
            self.notification_service.send_job_error(
                f"Single file processing failed for {file_name}: {str(e)}"
            )
            raise
    ...
    def _process_dropbox_files(self, max_files: int = 10):
        ...
        try:
            ...
            audio_video_files = self.dropbox_handler.get_audio_video_files(
                list(processed_jobs.keys())
            )
            ...
            for file_info in files_to_process:
                try:
                    ...
                    if result.get("success"):
                        ...
                        print(
                            f"❌ Failed to process: {file_info.get('name')} - {result.get('error')}"
                        )
            ...
            print(
                f"📊 Job completed: {processed_count}/{len(files_to_process)} files processed successfully"
            )
    ...
    def _load_job_tracking(self) -> dict[str, Any]:
        ...
        try:
            ...
            if blob.exists():
                ...
                print(
                    f"📥 Loaded job tracking from Cloud Storage: {len(processed_jobs)} processed files"
                )
            ...
            try:
                if self.job_tracking_file.exists():
                    with open(self.job_tracking_file) as f:
                        return json.load(f)
    ...
    def _save_job_tracking(self, processed_jobs: dict[str, Any]):
        ...
        try:
            ...
            print("💾 Saving job tracking to Cloud Storage...")
            ...
            try:
                ...
                try:
                    bucket = self.storage_client.create_bucket(
                        self.bucket_name, location="us-east1"
                    )
    ...
    def _prepare_audio_file(self, input_path: Path, file_name: str) -> Path:
        ...
        try:
            ...
            if is_audio_only:
                print(
                    f"🎵 Audio file {file_name} is {file_size / 1024 / 1024:.1f}MB - recompressing"
                )
            else:
                print(
                    f"🎬 Video file {file_name} is {file_size / 1024 / 1024:.1f}MB - extracting audio"
                )
