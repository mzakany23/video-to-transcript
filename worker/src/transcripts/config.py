...
class Config:
    ...
    TRANSCRIPTION_JOB_NAME: str = os.environ.get(
        "TRANSCRIPTION_JOB_NAME", "transcription-processor-dropbox"
    )
    ...
    NOTIFICATION_EMAIL: str = os.environ.get(
        "NOTIFICATION_EMAIL", ""
    )  # Email address to send notifications to
    ENABLE_EMAIL_NOTIFICATIONS: bool = (
        os.environ.get("ENABLE_EMAIL_NOTIFICATIONS", "false").lower() == "true"
    )
    ...
    def validate(cls) -> bool:
        ...
        if not (has_access_token or has_refresh_setup):
            required_vars.append(
                "DROPBOX_ACCESS_TOKEN or (DROPBOX_REFRESH_TOKEN + DROPBOX_APP_SECRET)"
            )
