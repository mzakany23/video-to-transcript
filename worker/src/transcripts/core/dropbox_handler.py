...
class DropboxHandler:
    ...
    def get_audio_video_files(self, processed_jobs: list[str] = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for file_entry in files:
                print(
                    f"🔍 Checking entry: {getattr(file_entry, 'name', 'NO_NAME')} (type: {type(file_entry).__name__})"
                )

                if not hasattr(file_entry, "path_display"):
                    print("  ❌ No path_display attribute")
                    continue
    ...
    def upload_transcript_results(
        self, transcript_data: dict, original_file_name: str
    ) -> dict[str, Any]:
        ...
        try:
            ...
            self.dbx.files_upload(
                json_content.encode("utf-8"), json_path, mode=dropbox.files.WriteMode.overwrite
            )
            ...
            txt_content = self._format_transcript_text(
                transcript_data, original_file_name, now.isoformat()
            )

            txt_path = f"{processing_folder}/{txt_filename}"

            self.dbx.files_upload(
                txt_content.encode("utf-8"), txt_path, mode=dropbox.files.WriteMode.overwrite
            )
    ...
    def _format_transcript_text(
        self, transcript_data: dict, original_file_name: str, timestamp: str
    ) -> str:
...
"""
    ...
    def get_folder_info(self) -> dict[str, str]:
        ...
        base_folder = (
            Config.RAW_FOLDER.split("/")[1]
            if Config.RAW_FOLDER.startswith("/") and len(Config.RAW_FOLDER.split("/")) > 1
            else "transcripts"
        )
        return {
