...
from services.storage import DropboxStorageProvider, StorageService
from services.transcription import OpenAITranscriptionProvider, TranscriptionService
...
class LegacyTranscriptionAdapter:
    ...
    def _initialize_services(self):
        ...
        try:
            ...
            self.transcription_service = TranscriptionService(
                transcription_provider=openai_provider, storage_provider=self.storage_service
            )
    ...
    def process_file(self, file_info: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                result = loop.run_until_complete(self._process_file_async(file_model))
            finally:
                loop.close()
        ...
        try:
            ...
            try:
                files = loop.run_until_complete(
                    self.storage_service.list_files(self.dropbox_provider.raw_folder, pattern="*")
                )
            finally:
                loop.close()
