...
from ...core.exceptions import AuthenticationError, JobError
from ...core.interfaces import JobRunner
from ...core.logging import get_logger
from ...core.models import JobSpec, JobState, JobStatus
...
class CloudRunJobRunner(JobRunner):
    ...
    def __init__(
        ...
        if not self.project_id:
            raise AuthenticationError("PROJECT_ID is required for Cloud Run jobs")
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            raise AuthenticationError(f"Failed to initialize Cloud Run client: {str(e)}")
    ...
    async def submit_job(self, spec: JobSpec) -> str:
        ...
        def _submit():
            try:
                ...
                raise JobError(f"Failed to submit job: {str(e)}")
        ...
        def _get_status():
            try:
                ...
                try:
                    ...
                    return JobStatus(
                        job_id=job_id, state=JobState.PENDING, metadata={"error": str(e)}
                    )
    ...
    async def list_jobs(
        ...
        def _list():
            try:
                ...
                # List executions
                execution_client = run_v2.ExecutionsClient()
                executions = execution_client.list_executions(parent=self.job_path, page_size=limit)
        ...
        def _get_logs():
            try:
                ...
                entries = logging_client.list_entries(
                    filter_=log_filter, order_by=cloud_logging.DESCENDING, max_results=lines
                )
