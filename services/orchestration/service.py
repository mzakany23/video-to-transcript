...
from ..core.models import JobSpec, JobStatus, JobState, ResourceRequirements
...
class OrchestrationService:
    ...
    async def submit_transcription_job(
        ...
        try:
            ...
            raise JobException(f"Job submission failed: {str(e)}")
    ...
    async def submit_batch_jobs(
        ...
        async def submit_single(file_info):
            async with semaphore:
                return await self.submit_transcription_job(
                    file_info["path"], 
                    file_info["name"]
                )
    ...
    async def wait_for_completion(
        ...
        while True:
            ...
            if elapsed > timeout_seconds:
                ...
                raise JobException(f"Job {job_id} timed out")
    ...
    async def get_job_logs(self, job_id: str, lines: int = 100) -> List[str]:
        ...
        try:
            ...
            if hasattr(self.job_runner, 'get_job_logs'):
                ...
                logger.warning(f"Log retrieval not supported by {self.job_runner.__class__.__name__}")
                return []
