...
from ..core.interfaces import StorageProvider, JobRunner
...
class WebhookService:
    ...
    def __init__(
        ...
        self.supported_formats = set(supported_formats or [
            '.mp3', '.mp4', '.mpeg', '.mpga', '.m4a', '.wav', '.webm',
            '.aac', '.oga', '.ogg', '.flac', '.mov', '.avi', '.mkv',
            '.wmv', '.flv', '.3gp'
        ])
        
        logger.info(f"Initialized WebhookService with {len(self.supported_formats)} supported formats")
    ...
    async def process_notification(
        ...
        try:
            ...
            if handler_type == "dropbox":
                ...
                raise ServiceException(f"Unknown webhook handler type: {handler_type}")
