...
class CursorManager:
    ...
    def __init__(
        self, storage_provider: StorageProvider, cursor_file_path: str = "webhook_cursors.json"
    ):
        """
    ...
    async def set_cursor(self, path: str, cursor: str):
        ...
        try:
            ...
            raise ServiceError(f"Failed to set cursor: {str(e)}")
    ...
    async def update_cursors(self, cursor_updates: dict[str, str]):
        ...
        try:
            ...
            raise ServiceError(f"Failed to update cursors: {str(e)}")
    ...
    async def reset_all_cursors(self):
        ...
        """
        try:
            empty_cursors = {"_reset_at": datetime.now().isoformat()}
    ...
    async def _save_cursors(self, cursors: dict[str, str]):
        ...
        def _save():
            try:
                ...
                if not result.success:
                    raise ServiceError(f"Failed to upload cursor file: {result.error}")
