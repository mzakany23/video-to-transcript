...
class JobTracker:
    ...
    def __init__(
        self, storage_provider: StorageProvider, tracking_file_path: str = "processed_jobs.json"
    ):
        """
    ...
    async def mark_processed(
        self, file_id: str, job_id: Optional[str] = None, file_info: Optional[dict[str, Any]] = None
    ):
        """
        ...
        try:
            ...
            raise ServiceError(f"Failed to mark file as processed: {str(e)}")
    ...
    async def list_processed_files(self, limit: Optional[int] = None) -> dict[str, dict[str, Any]]:
        """
        ...
        try:
            ...
            sorted_items = sorted(
                jobs.items(), key=lambda x: x[1].get("processed_at", ""), reverse=True
            )
    ...
    async def reset_tracking(self):
        ...
        """
        try:
            empty_jobs = {"_reset_at": datetime.now().isoformat()}
            ...
            raise ServiceError(f"Failed to reset job tracking: {str(e)}")
    ...
    async def cleanup_old_records(self, days_old: int = 30):
        ...
        try:
            ...
            if removed_count > 0:
                ...
                logger.info(
                    f"Cleaned up {removed_count} old job records (older than {days_old} days)"
                )
            else:
            ...
            raise ServiceError(f"Failed to cleanup old records: {str(e)}")
    ...
    async def _save_jobs(self, jobs: dict[str, dict[str, Any]]):
        ...
        def _save():
            try:
                ...
                if not result.success:
                    raise ServiceError(f"Failed to upload job tracking file: {result.error}")
