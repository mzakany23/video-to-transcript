...
class DropboxWebhookHandler:
    ...
    def _initialize_dropbox(self):
        ...
        try:
            ...
            raise ServiceError("Dropbox library not installed. Run: pip install dropbox")
        ...
        def _get_changes():
            try:
                ...
                for entry in result.entries:
                    logger.debug(
                        f"Change detected: {getattr(entry, 'name', 'NO_NAME')} "
                    ...
                    if not hasattr(entry, "path_display") or not entry.path_display.startswith(
                        self.raw_folder
                    ):
                    ...
                    if file_extension in self.supported_formats:
                        ...
                        file_info = {
                            ...
                            "modified": getattr(
                                entry, "client_modified", datetime.now()
                            ).isoformat(),
                        }
            ...
            for file_entry in files:
                ...
                if file_extension in self.supported_formats:
                    file_info = {
                        ...
                        "modified": getattr(
                            file_entry, "client_modified", datetime.now()
                        ).isoformat(),
                    }
    ...
    async def reset_cursors(self):
        ...
        try:
            ...
            raise ServiceError(f"Failed to reset cursors: {str(e)}")
