...
from ..core.interfaces import TranscriptionProvider, StorageProvider
from ..core.models import (
    TranscriptionOptions,
    TranscriptionResult,
    FileInfo,
)
from ..core.exceptions import TranscriptionException, StorageException
...
class TranscriptionService:
            ...
            if from_storage:
                if not self.storage:
                    raise TranscriptionException(
                        "Storage provider required for from_storage=True"
                    )
                ...
                if not download_result.success:
                    raise TranscriptionException(
                        f"Failed to download file: {download_result.error}"
                    )
    ...
    async def process_and_store(
        ...
        if not self.storage:
            raise TranscriptionException(
                "Storage provider required for process_and_store"
            )
        ...
        try:
            ...
            json_upload = await self.storage.upload_file(
                json_temp.name,
                json_path
            )
            ...
            txt_temp = tempfile.NamedTemporaryFile(
                mode='w',
                suffix='.txt',
                delete=False
            )
            ...
            txt_upload = await self.storage.upload_file(
                txt_temp.name,
                txt_path
            )
