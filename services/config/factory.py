...
from ..core.interfaces import JobRunner, StorageProvider, TranscriptionProvider
...
class ServiceFactory:
    ...
    def __init__(self, settings: Optional[Settings] = None):
        ...
        logger.info(
            f"ServiceFactory initialized with {len(self.get_available_providers())} providers"
        )
    ...
    def create_storage_provider(self, provider_name: Optional[str] = None) -> StorageProvider:
        ...
        if provider_name not in self.STORAGE_PROVIDERS:
            ...
            raise ServiceError(f"Unknown storage provider: {provider_name}. Available: {available}")

        config = self.settings.get_storage_config(provider_name)
        if not config.enabled:
            raise ServiceError(f"Storage provider '{provider_name}' is disabled")
        ...
        try:
            ...
            raise ServiceError(f"Storage provider creation failed: {str(e)}")
    ...
    def create_transcription_provider(
        ...
        if provider_name not in self.TRANSCRIPTION_PROVIDERS:
            ...
            raise ServiceError(
                f"Unknown transcription provider: {provider_name}. Available: {available}"
            )

        config = self.settings.get_transcription_config(provider_name)
        if not config.enabled:
            raise ServiceError(f"Transcription provider '{provider_name}' is disabled")
        ...
        try:
            ...
            raise ServiceError(f"Transcription provider creation failed: {str(e)}")
    ...
    def create_job_runner(self, runner_name: Optional[str] = None) -> JobRunner:
        ...
        if runner_name not in self.JOB_RUNNERS:
            ...
            raise ServiceError(f"Unknown job runner: {runner_name}. Available: {available}")

        config = self.settings.get_job_runner_config(runner_name)
        if not config.enabled:
            raise ServiceError(f"Job runner '{runner_name}' is disabled")
        ...
        try:
            ...
            raise ServiceError(f"Job runner creation failed: {str(e)}")

    def create_orchestration_service(self, job_runner_name: Optional[str] = None):
        """
    ...
    def create_storage_service(self, provider_name: Optional[str] = None):
        """
