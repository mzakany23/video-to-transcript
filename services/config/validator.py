...
class ConfigurationValidator:
        ...
        for provider_type, valid_providers in self.VALID_PROVIDERS.items():
            ...
            if not selected:
                errors.append(ValidationError(
                    field=setting_name,
                    error=f"{setting_name} is required",
                    severity="error"
                ))
    ...
    def _validate_job_runner(self) -> bool:
        ...
        if provider == "local":
            ...
            return bool(os.environ.get("PROJECT_ID") and 
...
class ConfigurationMigrator:
    ...
    def _migrate_from_0_1_0(self, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if storage_provider in storage_renames:
            ...
            logger.info(f"Renamed storage provider: {storage_provider} -> {storage_renames[storage_provider]}")
