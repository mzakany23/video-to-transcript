...
from ..core.models import DownloadResult, FileInfo, UploadResult
...
class StorageService:
    ...
    async def download_file(
        self, source_path: str, destination_path: Optional[str] = None
    ) -> DownloadResult:
        """
    ...
    async def upload_file(
        self, source_path: str, destination_path: str, metadata: Optional[dict[str, Any]] = None
    ) -> UploadResult:
        """
    ...
    async def file_exists(self, path: str) -> bool:
        ...
        try:
            ...
            raise StorageError(f"Existence check failed: {str(e)}")
    ...
    async def batch_download(
        self, file_paths: list[str], destination_dir: str
    ) -> list[DownloadResult]:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append(DownloadResult(success=False, error=str(result)))
            else:
    ...
    async def batch_upload(
        self, files: list[tuple[str, str]], metadata: Optional[dict[str, Any]] = None
    ) -> list[UploadResult]:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append(UploadResult(success=False, error=str(result)))
            else:
                processed_results.append(result)
