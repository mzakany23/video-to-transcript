...
from ...core.exceptions import AuthenticationError, StorageError
...
from ...core.resilience import retry_on_error, timeout
...
class GCSStorageProvider(StorageProvider):
    ...
    def __init__(
        ...
        if not self.bucket_name:
            raise StorageError("GCS_BUCKET is required for GCS storage")
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            raise AuthenticationError(f"Failed to initialize GCS client: {str(e)}")
    ...
    async def download(self, source_path: str, destination_path: str) -> DownloadResult:
        ...
        def _download():
            try:
                ...
                if not blob.exists():
                    raise StorageError(f"File not found in GCS: {source_path}")
                ...
                logger.info(
                    f"Downloaded from GCS: {source_path} -> {destination_path} ({file_size} bytes)"
                )
    ...
    async def upload(self, source_path: str, destination_path: str) -> UploadResult:
        ...
        def _upload():
            try:
                ...
                logger.info(
                    f"Uploaded to GCS: {source_path} -> {destination_path} ({file_size} bytes)"
                )
